...
async def test_scheme_files_crud(client, create_user, auth_headers) -> None:
    ...
    list_response = await client.post("/api/scheme-files/list", json={})
    ...
    delete_response = await client.post("/api/scheme-files/delete", json={"id": scheme_file["id"]}, headers=auth_headers)
...
async def test_scheme_files_are_isolated_per_user(client, create_user, auth_headers) -> None:
    ...
    delete_response = await client.post("/api/scheme-files/delete", json={"id": file_id}, headers=auth_headers)
...
async def test_scheme_file_content_can_be_empty_but_name_cannot(client, create_user, auth_headers) -> None:
    ...
    create_response = await client.post("/api/scheme-files/create", json={"name": "empty ok", "content": ""}, headers=auth_headers)
    assert create_response.status == 201

    bad_create = await client.post("/api/scheme-files/create", json={"name": "   ", "content": ""}, headers=auth_headers)
    assert bad_create.status == 400

    bad_save = await client.post("/api/scheme-files/save", json={"id": 999, "name": "   ", "content": ""}, headers=auth_headers)
...
async def test_deleting_missing_scheme_file_returns_404(client, create_user, auth_headers) -> None:
    ...
    delete_response = await client.post("/api/scheme-files/delete", json={"id": 999}, headers=auth_headers)
    assert delete_response.status == 404
